...
class UserRights {
	...
	static function getGlobalGroups(User $user) {
		...
		if ( $userId == 0 ) {
			...
			global $wgGlobalUserGroups, $wgSharedDB;
			...
			$dbr->freeResult($res);
			self::$globalGroup[$userId] = array_intersect($globalGroups, $wgGlobalUserGroups);
		}
	...
	static function addGlobalGroup( $user, $group ) {
		...
		if ( !in_array( $group, $wgGlobalUserGroups ) ) {
			return true;
		}
	...
	static function isCentralWiki() {
		global $wgIsCentralWiki;
		return (bool)$wgIsCentralWiki;
	}
